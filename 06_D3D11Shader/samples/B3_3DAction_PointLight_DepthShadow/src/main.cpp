// DxLib 公式サンプル「ポイントライトの深度シャドウ」の Direct3D 11 版。
// reference/d3d9_original/B_applied/3DAction_PointLight_DepthShadow から移植。Direct3D 9 版からの変更点には【D3D11】と書いています。
// ３Ｄアクション基本

#include "DxLib.h"
// 【D3D11】公式の原本にある #include "DxGraphics.h" は消した(SDK に無い DxLib 内部のヘッダーで、中身は使っていない)
#include <math.h>

// 【D3D11】頂点シェーダーの定数バッファ(b4)の中身。HLSL の cbuffer UserParam と同じ並びにする
// (MATRIX をそのまま入れる。HLSL 側は Direct3D 9 版と同じ matrix( column_major )のままで、同じ計算になる。仕様書 7 章)
struct LIGHTCAMERA_PARAM
{
	MATRIX ViewMatrix ;			// cfDepthViewMatrix0〜5
	MATRIX ProjectionMatrix ;	// cfDepthProjectionMatrix0〜5
} ;
int LightCameraParamCB ;
...
void Stage_Initialize( void )
{
	...
	Skin4_DepthShadow_Step1_VertexShader  = LoadVertexShader( "shaders/bin/SkinMesh4_DepthShadow_Step1VS.vso" ) ;
	Normal_DepthShadow_Step1_VertexShader = LoadVertexShader( "shaders/bin/NormalMesh_DepthShadow_Step1VS.vso" ) ;

	// 深度記録画像描画用のピクセルシェーダーを読み込む
	DepthShadow_Step1_PixelShader = LoadPixelShader( "shaders/bin/DepthShadow_Step1PS.pso" ) ;

	// 深度記録画像を使ったポイントライト一つの描画用頂点シェーダーを読み込む
	Skin4_PointLight_DepthShadow_Step2_VertexShader = LoadVertexShader( "shaders/bin/SkinMesh4_PointLight_DepthShadow_Step2VS.vso" ) ;
	Normal_PointLight_DepthShadow_Step2_VertexShader = LoadVertexShader( "shaders/bin/NormalMesh_PointLight_DepthShadow_Step2VS.vso" ) ;

	// 深度記録画像を使ったポイントライト一つの描画用ピクセルシェーダーを読み込む
	PointLight_DepthShadow_Step2_PixelShader = LoadPixelShader( "shaders/bin/PointLight_DepthShadow_Step2PS.pso" ) ;

	// 【D3D11】SetVSConstF などは Direct3D 11 では何もしないので、C++ から渡す値は定数バッファで渡す。ここで作っておく
	LightCameraParamCB = CreateShaderConstantBuffer( sizeof( LIGHTCAMERA_PARAM[ 6 ] ) ) ;
}
...
void DrawModelWithDepthShadow( void )
{
	...
	// 【D3D11】SetVSConstFMtx の代わりに、定数バッファ(b4)に入れて頂点シェーダーにセットする
	{
		LIGHTCAMERA_PARAM *Param = ( LIGHTCAMERA_PARAM * )GetBufferShaderConstantBuffer( LightCameraParamCB ) ;
		for( i = 0; i < 6; i++ )
		{
			Param[ i ].ViewMatrix = LightCamera_ViewMatrix[ i ] ;
			Param[ i ].ProjectionMatrix = LightCamera_ProjectionMatrix[ i ] ;
		}
		UpdateShaderConstantBuffer( LightCameraParamCB ) ;
		SetShaderConstantBuffer( LightCameraParamCB, DX_SHADERTYPE_VERTEX, 4 ) ;
	}
	...
	// 【D3D11】ResetVSConstF は不要(定数バッファは次に使うときまでそのままでよい)
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	// 【D3D11】Direct3D 11 を使用する(DxLib の既定)
	...
	// 【D3D11】作った定数バッファを削除する
	DeleteShaderConstantBuffer( LightCameraParamCB ) ;
