// SkinMesh4_DirLight_DepthShadow_Step2VS.fx の Direct3D 11 版(vs_4_0。1〜4 ボーンのスキニングメッシュ)
...
{
	matrix cfLightViewMatrix : packoffset( c0 ) ;	// ライトのワールド　→　ビュー行列
	matrix cfLightProjectionMatrix : packoffset( c4 ) ;	// ライトのビュー　　→　射影行列
} ;
