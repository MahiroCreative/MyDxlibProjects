// DxLib 公式サンプル「深度シャドウ」の Direct3D 11 版。
...
{
	MATRIX ViewMatrix ;			// cfLightViewMatrix
	MATRIX ProjectionMatrix ;	// cfLightProjectionMatrix
} ;
...
void Stage_Initialize( void )
{
	...
	Skin4_DirLight_DepthShadow_Step2_VertexShader = LoadVertexShader( "shaders/bin/SkinMesh4_DirLight_DepthShadow_Step2VS.vso" ) ;
	Normal_DirLight_DepthShadow_Step2_VertexShader = LoadVertexShader( "shaders/bin/NormalMesh_DirLight_DepthShadow_Step2VS.vso" ) ;

	// 深度記録画像を使ったディレクショナルライト一つの描画用ピクセルシェーダーを読み込む
	DirLight_DepthShadow_Step2_PixelShader = LoadPixelShader( "shaders/bin/DirLight_DepthShadow_Step2PS.pso" ) ;

	// 【D3D11】SetVSConstF などは Direct3D 11 では何もしないので、C++ から渡す値は定数バッファで渡す。ここで作っておく
	LightCameraParamCB = CreateShaderConstantBuffer( sizeof( LIGHTCAMERA_PARAM ) ) ;
}
...
void DrawModelWithDepthShadow( void )
{
	...
	// 【D3D11】SetVSConstFMtx の代わりに、定数バッファ(b4)に入れて頂点シェーダーにセットする
	{
		...
		Param->ViewMatrix = LightCamera_ViewMatrix ;
		Param->ProjectionMatrix = LightCamera_ProjectionMatrix ;
