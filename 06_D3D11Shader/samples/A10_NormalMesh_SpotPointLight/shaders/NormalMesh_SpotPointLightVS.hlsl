// NormalMesh_SpotPointLightVS.fx の Direct3D 11 版(vs_4_0。剛体メッシュ)
// reference/d3d9_original/A_base/10_NormalMesh_SpotPointLight から移植。Direct3D 9 版からの変更点には【D3D11】と書いています。
...
VS_OUTPUT main( VS_INPUT VSInput )
{
	...
	lTotalDiffuse += lLightGen * ( lLightLitDest.y * float4( g_Common.Light[ 0 ].Diffuse, 0.0f ) * g_Common.Material.Diffuse + g_Common.Light[ 0 ].Ambient ) ;

	// スペキュラライト蓄積値 += スペキュラ角度減衰計算結果 * 距離・スポットライト減衰 * ライトのスペキュラカラー
	lTotalSpecular += lLightGen * lLightLitDest.z * float4( g_Common.Light[ 0 ].Specular, 0.0f ) ;
