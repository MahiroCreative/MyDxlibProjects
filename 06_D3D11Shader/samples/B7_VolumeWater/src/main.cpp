// DxLib 公式サンプル「厚みのある水」の Direct3D 11 版。
...
// 【D3D11】ピクセルシェーダーの定数バッファ(b4)の中身。HLSL の cbuffer UserParam と同じ並び(1 個ずつ float4 の位置に置く)
struct VOLUMEWATER_PARAM
{
	FLOAT4 MaxOpacityDistance ;	// x だけ使う
	FLOAT4 MinOpacity ;			// x だけ使う
	FLOAT4 DepthTextureUseSize ;	// x, y だけ使う
} ;
int VolumeWaterParamCB ;
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	Depth_NormalMeshVS = LoadVertexShader( "shaders/bin/Depth_NormalMeshVS.vso" ) ;
	Depth_SkinMeshVS = LoadVertexShader( "shaders/bin/Depth_SkinMeshVS.vso" ) ;
	DepthPS = LoadPixelShader( "shaders/bin/DepthPS.pso" ) ;
	...
	VolumeWaterVS = LoadVertexShader( "shaders/bin/VolumeWaterVS.vso" ) ;
	VolumeWaterPS = LoadPixelShader( "shaders/bin/VolumeWaterPS.pso" ) ;

	// 【D3D11】SetVSConstF などは Direct3D 11 では何もしないので、C++ から渡す値は定数バッファで渡す。ここで作っておく
	VolumeWaterParamCB = CreateShaderConstantBuffer( sizeof( VOLUMEWATER_PARAM ) ) ;
	...
	while( ProcessMessage() == 0 )
	{
		...
		// 水面の描画
		{
			...
			VOLUMEWATER_PARAM *VolumeWaterParam = ( VOLUMEWATER_PARAM * )GetBufferShaderConstantBuffer( VolumeWaterParamCB ) ;
			...
			Param.z = 0.0f ;
			Param.w = 0.0f ;
			VolumeWaterParam->MaxOpacityDistance = Param ;	// 【D3D11】定数バッファ(b4)に入れる

			// 最低不透明度をセット
			Param.x = 0.1f ;
			VolumeWaterParam->MinOpacity = Param ;	// 【D3D11】定数バッファ(b4)に入れる

			// 深度テクスチャの使用しているサイズをセット
			Param.x = ( float )SCREEN_W / DEPTH_SCREEN_W ;
			Param.y = ( float )SCREEN_H / DEPTH_SCREEN_H ;
			VolumeWaterParam->DepthTextureUseSize = Param ;	// 【D3D11】定数バッファ(b4)に入れる

			// 【D3D11】定数バッファを GPU に送って、ピクセルシェーダーにセットする
			UpdateShaderConstantBuffer( VolumeWaterParamCB ) ;
			SetShaderConstantBuffer( VolumeWaterParamCB, DX_SHADERTYPE_PIXEL, 4 ) ;
	...
	DeleteShaderConstantBuffer( VolumeWaterParamCB ) ;
