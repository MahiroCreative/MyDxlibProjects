// DxLib 関数リファレンス SetVSConstFArray のサンプルの Direct3D 11 版。
// Tex1.bmp を貼り付けたポリゴン２枚を、予め設定した画面４隅の座標に順番に描画します。
//
// Direct3D 9 版(reference/d3d9_original/C_function/C4_SetVSConstFArrayTest)からの変更点には【D3D11】と書いています。
#include "DxLib.h"

// 【D3D11】頂点シェーダーの定数バッファ(b4)の中身。HLSL の cbuffer と同じ並びにする(大きさは 16 バイトの倍数)
struct VS_PARAM
{
    FLOAT4 DrawPosition[ 4 ] ;  // 座標値の配列
    int PositionIndex[ 4 ] ;    // 座標値配列のインデックス(HLSL の int4。使うのは [ 0 ] だけ)
} ;
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
    ...
    // ピクセルシェーダーを読み込む
    pshandle = LoadPixelShader( "shaders/bin/SetVSConstFArrayTestPS.pso" ) ;

    // 描画に使用する画像の読み込み
    texhandle = LoadGraph( "Tex1.bmp" ) ;

    // 【D3D11】SetVSConstF / SetVSConstFArray は Direct3D 11 では何もしないので、定数バッファを作って値を渡す
    ...
    pos[ 3 ].z = 0.0f ;
    pos[ 3 ].w = 0.0f ;
    // 【D3D11】SetVSConstFArray の代わりに、定数バッファ(b4)の DrawPosition にコピーする
    for( int i = 0 ; i < 4 ; i ++ )
    {
        vsparam->DrawPosition[ i ] = pos[ i ] ;
    }
    ...
    while( ProcessMessage() == 0 && CheckHitKey( KEY_INPUT_ESCAPE ) == 0 )
    {
        ...
        // 【D3D11】位置番号を定数バッファ(b4)の PositionIndex にセットして、GPU に送る
        // (Direct3D 9 では int4 の定数も FLOAT4 として SetVSConstF で渡していたが、Direct3D 11 では int をそのまま入れる)
        vsparam->PositionIndex[ 0 ] = posind ;
    ...
    // 【D3D11】ResetVSConstF / ResetVSConstI の代わりに、作った定数バッファを削除する
