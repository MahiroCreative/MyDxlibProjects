// DxLib 公式サンプル「オリジナルシェーダーを使用した３Ｄモデルの描画基本」の「剛体メッシュのライティング無し描画」の Direct3D 11 版。
// reference/d3d9_original/A_base/01_NormalMesh_NoLight から移植。Direct3D 9 版からの変更点には【D3D11】と書いています。
#include "DxLib.h"
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	VertexShaderHandle = LoadVertexShader( "shaders/bin/NormalMesh_NoLightVS.vso" ) ;

	// ピクセルシェーダーを読み込む
	PixelShaderHandle = LoadPixelShader( "shaders/bin/NormalMesh_NoLightPS.pso" ) ;
