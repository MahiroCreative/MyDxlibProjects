...
| サンプル | 内容 | 確認 |
|---|---|---|
| D1_Sprite2DVertexShader | 2D の絵を頂点シェーダーでなびかせる。正射影カメラ + 3D の板(`src/Camera2D.h`)で、2D と同じ見た目のまま自作の頂点シェーダーを使う(仕様書 3.4) | `verify/` の ortho2d(`DrawGraph` と画素単位で一致など 10 項目)。`run.py` では動くことだけ |
