// DxLib 公式サンプル「被写界深度」の Direct3D 11 版。
// reference/d3d9_original/B_applied/DepthOfField から移植。Direct3D 9 版からの変更点には【D3D11】と書いています。
#include "DxLib.h"
#include <math.h>

int PSParamCB ;		// 【D3D11】ピクセルシェーダーの定数バッファ(b4)。中身は FLOAT4 1 個
...
void DrawScreen( int ScreenGraphHandle, int UseDepthShader )
{
	...
	if( UseDepthShader != 0 )
	{
		...
		// 【D3D11】ピクセルシェーダーの定数バッファ(b4)にセット
		*( FLOAT4 * )GetBufferShaderConstantBuffer( PSParamCB ) = ParamF ;
		UpdateShaderConstantBuffer( PSParamCB ) ;
		SetShaderConstantBuffer( PSParamCB, DX_SHADERTYPE_PIXEL, 4 ) ;
	}
	else
	{
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	DepthOfFieldPS     = LoadPixelShader( "shaders/bin/DepthOfFieldPS.pso" ) ;

	// 【D3D11】SetVSConstF などは Direct3D 11 では何もしないので、C++ から渡す値は定数バッファで渡す。ここで作っておく
	PSParamCB = CreateShaderConstantBuffer( sizeof( FLOAT4 ) ) ;
	...
	DeleteShaderConstantBuffer( PSParamCB ) ;
