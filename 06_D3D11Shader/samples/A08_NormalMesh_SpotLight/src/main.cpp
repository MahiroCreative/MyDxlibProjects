// DxLib 公式サンプル「オリジナルシェーダーを使用した３Ｄモデルの描画基本」の「剛体メッシュのスポットライトあり描画」の Direct3D 11 版。
// reference/d3d9_original/A_base/08_NormalMesh_SpotLight から移植。Direct3D 9 版からの変更点には【D3D11】と書いています。
#include "DxLib.h"
#include <math.h>
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	VertexShaderHandle = LoadVertexShader( "shaders/bin/NormalMesh_SpotLightVS.vso" ) ;

	// ピクセルシェーダーを読み込む
	PixelShaderHandle = LoadPixelShader( "shaders/bin/NormalMesh_SpotLightPS.pso" ) ;
