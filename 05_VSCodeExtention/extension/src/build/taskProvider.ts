...
export class DxLibTaskProvider implements vscode.TaskProvider {
	...
	private async createTask(def: DxLibTaskDefinition, folder: vscode.WorkspaceFolder, base?: vscode.Task): Promise<vscode.Task> {
		...
		const task = new vscode.Task(base?.definition ?? def, folder, taskLabel(def.config), TASK_TYPE, execution, ['$msCompile']);
	...
	private async writeBuildScript(folder: vscode.WorkspaceFolder, config: BuildConfig): Promise<string> {
		const dir = path.join(this.context.globalStorageUri.fsPath, 'build');
		fs.mkdirSync(dir, { recursive: true });
		const hash = crypto.createHash('md5').update(folder.uri.fsPath.toLowerCase()).digest('hex').slice(0, 8);
		const script = path.join(dir, `${projectExeName(folder)}_${hash}_${config}.bat`);
		...
		if (vs.state !== 'ok' || !vs.vcvarsall) {
			...
			lines.push(...buildLines(folder, config, vs.vcvarsall, sdk.path));
		}
...
function buildLines(folder: vscode.WorkspaceFolder, config: BuildConfig, vcvarsall: string, sdkPath: string): string[] {
	...
	return [
		...
		`cl ${common} ${configFlags} /I "%SDK%" /I "%PROJ%\\src" !SRCS! /Fo"%OUT%\\obj\\\\" /Fd"%OUT%\\${exeName}.pdb" /Fe"%OUT%\\${exeName}.exe" /link /SUBSYSTEM:WINDOWS /LIBPATH:"%SDK%" ${linkFlags}`,
		'if errorlevel 1 (',
